...
if (isset($_GET['id'])) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<script>alert('News item not found.'); window.location.href='admin_upload_news.php';</script>";
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    // Prepare the update query for title and content
        ...
        if ($news_image['size'] > 0) { // Check if a new image is uploaded
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($news_image["tmp_name"], $target_file)) {
                    ...
                    $update_query_image = "UPDATE news SET image = '$target_file' WHERE id = $id";
                    if (mysqli_query($conn, $update_query_image)) {
                        echo "<script>alert('News updated successfully.'); window.location.href='admin_upload_news.php';</script>";
                    } else {
...
<div class="container mt-5">
    ...
    <?php if ($editMode): ?>
    <form action="admin_edit_upload_news.php" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="content" id="content" class="form-control" rows="4" required><?php echo $content; ?></textarea>
        </div>
        <div class="mb-3">
            ...
            <?php if (!empty($image_path)): ?>
                <p><img src="uploads/<?php echo $image_path; ?>" alt="Current Image" class="img-fluid" style="max-width: 200px;">
                </p>
            <?php endif; ?>
        </div>
