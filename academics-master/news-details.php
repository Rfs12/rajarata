...
if ($id > 0) {
    // Prepare the SQL query to find the news item across all tables
    $sql = "
        SELECT id, title, content, image, created_at FROM sports_news WHERE id = $id
        UNION ALL
        SELECT id, title, content, image, created_at FROM academics_news WHERE id = $id
        UNION ALL
        SELECT id, title, content, image, created_at FROM genaral_news WHERE id = $id
        UNION ALL
        SELECT id, title, content, image, created_at FROM news WHERE id = $id
    ";
...
<body>
  ...
  <div class="site-section">
    <div class="container">
      ...
      <?php if (!empty($image)): ?>
        <img src="uploads/<?php echo $image; ?>" alt="Image" class="img-fluid">
    <?php else: ?>
        <img src="images/default-image.jpg" alt="Default Image" class="img-fluid">
    <?php endif; ?>
